...
MAX_PROXIES = 50000
SKIP_FIRST = 50000  # Skip first 50k, take second 50k (50k-100k)
CONCURRENCY = 100
TIMEOUT = 10
...
async def main():
    print("[Boost-1] Starting ProxRipper HTTP booster SECOND 50k...")
    print(f"[Boost-1] Config: SKIP={SKIP_FIRST}, MAX={MAX_PROXIES}, CONCURRENCY={CONCURRENCY}, TIMEOUT={TIMEOUT}s")

    # 1. Load dead set (persistent, never deleted)
    dead_set = load_dead_set()
    print(f"[Boost] Loaded dead list: {len(dead_set)} proxies")

    # 2. Fetch ProxRipper HTTP (second 50k: skip first 50k)
    print(f"[Boost] Fetching ProxRipper HTTP (second 50k: {SKIP_FIRST}-{SKIP_FIRST+MAX_PROXIES})...")
    text = ""
    try:
    ...
    # Parse second 50k proxies - skip first 50k, take next 50k (dedup while preserving order)
    all_proxies = []
    seen = set()
    ...
    print(f"[Boost] Fetched {len(all_proxies)} total unique, using second 50k: {SKIP_FIRST}-{SKIP_FIRST+len(proxies)} ({len(proxies)} proxies)")
